...
it("has left-arrow hidden when index is 0", () => {
	const { container } = render(<Carousel photos={TEST_IMAGES} title="images for testing" />);
	const leftArrow = container.querySelector(".bi-arrow-left-circle.toggle-off");
	expect(leftArrow).toBeInTheDocument();
});

it("has right-arrow hidden when index is last", () => {
	const { container } = render(<Carousel photos={TEST_IMAGES} title="images for testing" />);
	...
	rightArrow = container.querySelector(".bi-arrow-right-circle.toggle-off");
